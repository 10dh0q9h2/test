...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const getVocabLookupForms = (entry) => {
    return entry.word
      .split('/')
      .flatMap(part => part.split(','))
      .map(part => normalizeLookupText(part.replace(/\([^)]*\)/g, '')))
      .filter(Boolean);
  };

  const lookupWord = (clickedWord) => {
    const cleanPhrase = normalizeLookupText(clickedWord);
    const cleanWord = cleanPhrase.replace(/\s+/g, '');
    if (!cleanPhrase || cleanWord.length < 2) return null;
...
2. ["번역문1", "번역문2", ...] 형식의 JSON 배열만 출력할 것. 마크다운(\`\`\`) 없이 출력.`;
  ...
  const renderActiveQuiz = (quizObj) => {
    ...
    if (quizObj.options && quizObj.options.length > 0) {
      ...
      poolDiv.innerHTML = '<strong>보기:</strong> <span class="vocab-options-hint">단어를 누르면 뜻이 표시됩니다.</span>';
      quizObj.options.forEach(opt => {
        ...
        span.setAttribute('title', '뜻 보기');
        span.addEventListener('click', (e) => {
          e.stopPropagation();
          showDictTooltip(span, opt, quizObj.title);
        });
  ...
  const getAcceptedAnswers = (answer) => {
    ...
    if (parenMatch) {
      ...
      addAnswer(base);
      inner
        .replace(/^(?:또는|\bor\b)\s*/i, '')
        .split(/\s*(?:또는|\bor\b|,|;)\s*/i)
        .forEach(addAnswer);

      if (base.toLowerCase() === 'x') {
        answers.add('');
      }
    }
  ...
  const formatAnswerForFeedback = (answer) => {
    const raw = (answer || '').trim();
    const parenMatch = raw.match(/^(.+?)\s*\(([^)]+)\)\s*$/);
    if (!parenMatch) return raw;
  ...
  const gradeQuiz = (quizObj) => {
    ...
    inputs.forEach(input => {
      ...
      const userAnswer = normalizeQuizAnswer(input.value);
      ...
      if (isCorrect) {
        ...
        feedback.innerHTML = `<i class="fa-solid fa-circle-xmark"></i> 오답 (정답: <strong>${formatAnswerForFeedback(correctAnswer)}</strong>)`;
      }
