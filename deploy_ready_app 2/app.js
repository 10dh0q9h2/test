  ...
  const loadData = async () => {
    try {
      ...
      const [vocabRes, grammarRes, textRes, quizRes, textTransRes, ctxRes] = await Promise.all([
        fetch('단어장_05.20.md').then(r => { if (!r.ok) throw new Error(); return r.text(); }),
        fetch('문법_05.20.md').then(r => { if (!r.ok) throw new Error(); return r.text(); }),
        fetch('본문_05.20.md').then(r => { if (!r.ok) throw new Error(); return r.text(); }),
        fetch('연습문제_05.20.md').then(r => { if (!r.ok) throw new Error(); return r.text(); }),
        fetch('본문해석_05.20.json').then(r => r.ok ? r.json() : null).catch(() => null),
        ...
        dashboardBody.innerHTML = `
          <div class="panel" style="border-color: var(--color-danger); background-color: rgba(239, 68, 68, 0.05); padding: 32px; text-align: center;">
            ...
            <p style="color: var(--text-secondary); max-width: 600px; margin: 0 auto 20px auto; line-height: 1.6;">
              브라우저의 보안 정책(CORS)으로 인해 로컬 파일(index.html)을 직접 더블 클릭하여 실행하면 마크다운 데이터를 읽어올 수 없습니다. 
            ...
            <p style="color: var(--text-tertiary); font-size: 12px;">
              또는 마크다운 파일들의 이름이 정확히 일치하는지 확인해 주세요 (단어장_05.20.md, 문법_05.20.md, 본문_05.20.md, 연습문제_05.20.md).
            </p>
          </div>
        `;
      }
